...
from rag_project.logger import logging  # Using the logger module
from urllib.parse import quote_plus
import numpy as np
from langchain.text_splitter import RecursiveCharacterTextSplitter  # New splitter
from rag_project.exception import CustomException  # Import CustomException
...
try:
    ...
    logging.info(f"📂 Loaded {len(documents)} documents from pickle file.")
...
def generate_embedding(text, retries=3):
    try:
        if len(text) > 50000:
            logging.warning(f"⚠️ Skipping overly long chunk ({len(text)} characters).")
            return None
        ...
        for attempt in range(retries):
            try:
                ...
                logging.error(f"Error generating embedding (Attempt {attempt+1}): {e}")
                if attempt == retries - 1:
                    raise CustomException(e, sys)
    except Exception as e:
        logging.error(f"❌ Unexpected error in embedding generation: {e}")
        raise CustomException(e, sys)
...
try:
    ...
    logging.info(f"🧩 Created {len(chunked_documents)} text chunks using RecursiveCharacterTextSplitter.")
except Exception as e:
    logging.error(f"❌ Error in chunking documents: {e}")
    raise CustomException(e, sys)
...
try:
    ...
    logging.info("✅ Vector embeddings saved to MongoDB.")
except Exception as e:
